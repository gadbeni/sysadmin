...
@section('content')
    ...
    @else
        <div class="content">
            ...
            <p>Posteriormente, mediante solicitud de fecha, {{ date('d', strtotime($addendums->first()->request_date)) }} de {{ $months[intval(date('m', strtotime($addendums->first()->request_date)))] }} de {{ date('Y', strtotime($addendums->first()->request_date)) }} emitido por {{ $applicant->person->first_name }} {{ $applicant->person->last_name }}/{{ $applicant->cargo ? $applicant->cargo->Descripcion : $applicant->job ? $applicant->job->name : Str::upper($applicant->job_description) }}, solicita aprobación de la ampliación del contrato de Consultoría Individual de línea <b>"{{ Str::upper($contract->cargo->Descripcion).($contract->name_job_alt ? ' - '.$contract->name_job_alt : '') }} PARA LA/EL {{ Str::upper($contract->work_location ?? $contract->unidad_administrativa->nombre) }} DEL SEDEGES-BENI"</b>, estableciendo que es necesario seguir contando con esta consultoría toda vez que este personal, cuenta con amplia experiencia de conocimiento y capacidad en el área.</p>
