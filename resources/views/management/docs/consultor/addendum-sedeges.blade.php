...
@php
    ...
    $applicant = $addendums->first()->applicant;
@endphp

@section('qr_code')
    <div id="qr_code" >
        @php
            $qrcode = QrCode::size(70)->generate(($number_addendum == 2 ? 'SEGUNDO' : '')." CONTRATO MODIFICATORIO DE CONSULTORIA INDIVIDUAL DE LINEA ".($signature ? $signature->direccion_administrativa->sigla : 'UJ/SDAF')." NRO ".($addendums->first()->code)." RELACIONADO AL CONTRATO ".($signature ? $signature->direccion_administrativa->sigla : 'UJ/SDAF')."/GAD-BENI NRO ".$code);
        @endphp
        {!! $qrcode !!}
    </div>
@endsection
...
@section('content')
    ...
    @else
        <div class="content">
            ...
            @if ($number_addendum == 2)
            @php
                $contract_duration = contract_duration_calculate($addendums->last()->start, $addendums->last()->finish);
            @endphp
            <p>En fecha <b>{{ date('d', strtotime($addendums->last()->start)).' de '.$months[intval(date('m', strtotime($addendums->last()->start)))].' de '.date('Y', strtotime($addendums->last()->start)) }}</b> se firma el primer contrato modificatorio <b>{{ $addendums->last()->code }}</b> al contrato administrativo <b>{{ $code }}</b> que amplia por {{ $contract_duration->months }} meses {{ $contract_duration->days ? ' y '.$contract_duration->days.' días' : '' }} la Consutoría Individual de Línea <b>{{ Str::upper($contract->cargo->Descripcion).($contract->name_job_alt ? ' - '.$contract->name_job_alt : '') }}</b> a cargo de {{ $contract->person->gender == 'masculino' ? 'el señor' : 'la señora' }} <b>{{ $contract->person->first_name }} {{ $contract->person->last_name }}</b>.</p>
            @endif
            <p>Posteriormente, mediante solicitud de fecha, {{ date('d', strtotime($addendums->first()->request_date)) }} de {{ $months[intval(date('m', strtotime($addendums->first()->request_date)))] }} de {{ date('Y', strtotime($addendums->first()->request_date)) }} emitido por {{ $applicant->person->first_name }} {{ $applicant->person->last_name }}/{{ $applicant->cargo ? $applicant->cargo->Descripcion : ($applicant->job ? $applicant->job->name : Str::upper($applicant->job_description)) }}, solicita aprobación de la {{ $number_addendum == 2 ? 'segunda' : '' }} ampliación del contrato de Consultoría Individual de línea <b>"{{ Str::upper($contract->cargo->Descripcion).($contract->name_job_alt ? ' - '.$contract->name_job_alt : '') }} PARA LA/EL {{ Str::upper($contract->work_location ?? $contract->unidad_administrativa->nombre) }} DEL SEDEGES-BENI"</b>, estableciendo que es necesario seguir contando con esta consultoría toda vez que este personal, cuenta con amplia experiencia de conocimiento y capacidad en el área.</p>
            ...
            <p><b>CLÁUSULA QUINTA. - (DE LOS ANEXOS)</b> Son parte del presente contrato como anexos, el contrato principal suscrito en fecha <b>{{ date('d', strtotime($contract->start)) }} de {{ $months[intval(date('m', strtotime($contract->start)))] }} de {{ date('Y', strtotime($contract->start)) }}</b>, solicitud de ampliación realizada por {{ $applicant->person->first_name }} {{ $applicant->person->last_name }}/{{ $applicant->cargo ? $applicant->cargo->Descripcion : ($applicant->job ? $applicant->job->name : Str::upper($applicant->job_description)) }}.</p>
