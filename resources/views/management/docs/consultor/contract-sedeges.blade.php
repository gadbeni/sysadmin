...
@section('qr_code')
    <div id="qr_code">
        {!! QrCode::size(80)->generate('Consultoría de Línea '.$code.' '.$contract->person->first_name.' '.$contract->person->last_name.' con C.I. '.$contract->person->ci.', del '.date('d', strtotime($contract->start)).' de '.$months[intval(date('m', strtotime($contract->start)))].' al '.date('d', strtotime($contract->finish)).' de '.$months[intval(date('m', strtotime($contract->finish)))].' de '.date('Y', strtotime($contract->finish)).' con un sueldo de '.number_format($contract->cargo->nivel->where('IdPlanilla', $contract->cargo->idPlanilla)->first()->Sueldo, 2, ',', '.').' Bs.'); !!}
    </div>
@endsection

@section('content')
    <div class="content" style="text-align: justify">
        <h2 class="text-center" style="font-size: 15px">CONTRATO ADMINISTRATIVO SEDEGES-BENI N&deg; {{ $code }} <br> "CONSULTOR INDIVIDUAL DE LÍNEA {{ Str::upper($contract->cargo->Descripcion).($contract->name_job_alt ? ' - '.$contract->name_job_alt : '') }} {{ ($contract->unidad_administrativa ? ' PARA LA/EL '.Str::upper($contract->unidad_administrativa->nombre).' DEL ' : '') }} SEDEGES-BENI" </h2>
        ...
        <p>
            <ul>
                ...
                <li><p>Ley del Presupuesto General del Estado aprobado para la Gestión 2022 y su Reglamentación.</p></li>
                <li><p>Otras disposiciones conexas.</p></li>
            </ul>
        </p>
        <p><b>CLÁUSULA TERCERA.- (OBJETO Y CAUSA)</b> El objeto y causa del presente Contrato es la prestación del servicio de "<b>CONSULTOR INDIVIDUAL DE LÍNEA {{ Str::upper($contract->cargo->Descripcion).($contract->name_job_alt ? ' - '.$contract->name_job_alt : '') }} {{ ($contract->unidad_administrativa ? ' PARA LA/EL '.Str::upper($contract->unidad_administrativa->nombre).' DEL ' : '') }}  SEDEGES-BENI</b>", que en adelante se denominará la <b>CONSULTORÍA</b>, provisto por <b>{{ $contract->person->gender == 'masculino' ? 'el CONSULTOR' : 'la CONSULTORA' }}</b> de conformidad con los Términos de Referencia, con estricta y absoluta sujeción al presente Contrato.</p>
        <p><b>CLÁUSULA CUARTA.- (DOCUMENTOS INTEGRANTES DEL CONTRATO)</b> Forman parte del presente Contrato, los siguientes documentos:</p>
        {{-- <p>
            <ol type="a">
                <li><p>Declaración Jurada de No Doble Percepción.</p></li>
                <li><p>Términos de Referencia.</p></li>
                <li><p>Informe de Evaluación y Recomendación del proceso de contratación N° .</p></li>
                <li><p>Certificación Presupuestaria (Preventivo N° )</p></li>
                <li><p>Certificación Poa N° .</p></li>
                <li><p>Declaración Jurada de No Incompatibilidad Legal.</p></li>
                <li><p>Fotocopia de Cédula de Identidad.</p></li>
                <li><p>Certificación de Programación Operativa Anual (P.O.A.)</p></li>
                <li><p>Curriculum Vitae</p></li>
                <li><p>Certificado de Antecedentes Penales (REJAP)</p></li>
                <li><p>Certificado de No Violencia (Ley 1153)</p></li>
                <li><p>Certificado de Inscripción NIT</p></li>
                <li><p>Certificación de No Adeudo (AFP)</p></li>
                <li><p>Certificado del RUPE N°.( cuando corresponda).</p></li>
            </ol>
        </p> --}}
        ...
        <p><strong><span>CL&Aacute;USULA SEXTA.- (VIGENCIA) </span></strong><span>El Contrato, entrar&aacute; en vigencia desde el d&iacute;a siguiente h&aacute;bil de su suscripci&oacute;n, por ambas partes, hasta que las mismas hayan dado cumplimiento a todas las Cl&aacute;usulas contenidas en el presente Contrato.</span></p>
        ...
        <p><strong><span>CL&Aacute;USULA D&Eacute;CIMA.- (LUGAR DE PRESTACI&Oacute;N DE SERVICIOS)</span></strong> <span><b>{{ $contract->person->gender == 'masculino' ? 'El CONSULTOR' : 'La CONSULTORA' }}</b>  realizar&aacute; la <strong>CONSULTOR&Iacute;A</strong>, objeto del presente Contrato en<strong> {{ $contract->work_location }} </strong>, <span>cumpliendo con los horarios establecidos del SEDEGES - BENI.</p>
