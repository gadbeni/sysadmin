...
@section('content')
    ...
    @else
        <div class="content">
            <div class="page-head">
                <h3>CONTRATO ADMINISTRATIVO DE SERVICIO DE CONSULTORÍA DE LÍNEA <br> {{ $signature ? $signature->direccion_administrativa->nombre : 'SECRETARÍA DEPARTAMENTAL DE ADMINSTRACIÓN Y FINANZAS' }} <br>GAD-BENI N&deg; {{ $code }}</h3>
            </div>
            ...
            <p>Correr&aacute; por cuenta <strong>{{ $contract->person->gender == 'masculino' ? 'DEL CONSULTOR' : 'DE LA CONSULTORA' }}</strong> el pago de la su AFP y al momento de solicitar la cancelaci&oacute;n de sus haberes mensuales deber&aacute; adjuntar el formulario mediante el cual se evidencie la cancelaci&oacute;n del mismo.</p>
