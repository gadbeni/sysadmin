...
@section('content')
    ...
    @else
        <div class="content">
            <div class="page-head">
                <h3>CONTRATO ADMINISTRATIVO DE SERVICIO DE CONSULTORÍA INDIVIDUAL DE LÍNEA <br> {{ $signature ? $signature->direccion_administrativa->nombre : 'SECRETARÍA DEPARTAMENTAL DE ADMINSTRACIÓN Y FINANZAS' }} <br>GAD-BENI N&deg; {{ $code }}</h3>
            </div>
            ...
            <p>En el Decreto Supremo N&deg; 0181, de 28 de junio de 2009, de las Normas B&aacute;sicas del Sistema de Administraci&oacute;n de Bienes y Servicios NB-SABS y los T&eacute;rminos de Referencia (TDR), invito {{ $contract->person->gender == 'masculino' ? 'al Señor' : 'a la Señora' }} {{ $contract->person->first_name }} {{ $contract->person->last_name }}</strong>, para que preste los servicios de Consultor&iacute;a Individual de L&iacute;nea para el cargo de <strong>&ldquo;{{ Str::upper($contract->cargo->Descripcion.($contract->name_job_alt ? ' - '.$contract->name_job_alt : '')) }}&rdquo;</strong>, con cargo al {{ ucfirst($contract->program->class) }} <strong>&ldquo;{{ Str::upper($contract->program->name) }}&rdquo;</strong>, de la/el <strong>{{ Str::upper($contract->unidad_administrativa->nombre) }} </strong>dependiente de la/el <strong>{{ Str::upper($contract->direccion_administrativa->nombre) }},</strong><strong> </strong>quien en adelante se denominara <strong>LA CONSULTORIA</strong>, provistos por <strong>{{ $contract->person->gender == 'masculino' ? 'EL CONSULTOR' : 'LA CONSULTORA' }}</strong> de conformidad a t&eacute;rminos de referencia, con estricta y absoluta sujeci&oacute;n a este contrato.</p>
            ...
            <p>El objeto y causa del presente contrato es contratar los servicios de un Consultor Individual de L&iacute;nea en el cargo de <strong>&ldquo;{{ Str::upper($contract->cargo->Descripcion.($contract->name_job_alt ? ' - '.$contract->name_job_alt : '')) }}&rdquo;</strong> para dar apoyo a la/el <strong>{{ Str::upper($contract->unidad_administrativa->nombre) }}</strong><strong> </strong>dependiente de la <strong>{{ Str::upper($contract->direccion_administrativa->nombre) }}.</strong></p>
            ...
            <ul>
                ...
                <li>
                    <p>Certificado de inscripción a la GESTORA pública.</p>
                </li>
                <li>
                    <p>Aprobación del proceso de contratación.</p>
                </li>
                <li>
            ...
            <p>El contrato entrará en vigencia  partir de la suscripción del mismo, por ambas partes, hasta que las mismas hayan dado cumplimiento a todas las cláusulas contenidas en el presente contrato, duración computable a partir del <strong>{{ date('d', strtotime($contract->start)) }} de {{ $months[intval(date('m', strtotime($contract->start)))] }} al {{ date('d', strtotime($contract_finish)) }} de {{ $months[intval(date('m', strtotime($contract_finish)))] }} de {{ date('Y', strtotime($contract->start)) }}.</strong></p>
            ...
            <p>De conformidad a la <b>LEY 396</b> de fecha 26 de agosto de 2013, LEY DE MODIFICACIONES AL PRESUPUESTO GENERAL DEL ESTADO, que en su Art. 5 (CONTRATACIONES DE CONSULTORÍA) parágrafo III Inc. e), estable que; "Se autorizan los pagos de pasajes y viáticos para los Consultores Individuales de Línea, siempre que dicha actividad, se halle prevista en el referido contrato de los gatos por pasajes, viáticos que se requieran para el desarrollo de las actividades {{ $contract->person->gender == 'masculino' ? 'del CONSULTOR' : 'de la CONSULTORA' }} en la provincias y/o capitales de Bolivia", serán proporcionados de acuerdo a la existencia de presupuesto.</p>
            <p><span style="text-decoration: underline;"><strong>CL&Aacute;USULA D&Eacute;CIMA</strong></span><strong>. - (PAGO DE LA GESTORA.) </strong></p>
            <p>Correr&aacute; por cuenta <strong>{{ $contract->person->gender == 'masculino' ? 'DEL CONSULTOR' : 'DE LA CONSULTORA' }}</strong> el pago de la GESTORA pública y al momento de solicitar la cancelaci&oacute;n de sus haberes mensuales deber&aacute; adjuntar el formulario mediante el cual se evidencie la cancelaci&oacute;n del mismo.</p>
            ...
            <p>Correr&aacute; por cuenta <strong>{{ $contract->person->gender == 'masculino' ? 'DEL CONSULTOR' : 'DE LA CONSULTORA' }}</strong>el pago de todos los impuestos vigentes en el pa&iacute;s.</p>
            ...
            <p>La modificación al contrato podrá realizarse hasta un máximo de dos (2) veces, no debiendo exceder el plazo de cada modificación al establecido en el presente contrato, de acuerdo con lo establecido en el Artículo 89 del Decreto Supremo N° 0181, de 28 de junio de 2009, de las Normas Básicas del Sistema de Administración de Bienes y Servicios – NB-SABS.</p>
            ...
            <p><strong>2.2 A requerimiento </strong><strong> {{ $contract->person->gender == 'masculino' ? 'DEL CONSULTOR' : 'DE LA CONSULTORA' }}</strong><strong>, por causales atribuibles a LA ENTIDAD: </strong></p>
            ...
            <p><strong>2.3 Por causas de fuerza mayor o caso fortuito que afecten a LA ENTIDAD:</strong></p>
            ...
            <p><span style="text-decoration: underline;"><strong>CL&Aacute;USULA D&Eacute;CIMA OCTAVA</strong></span><strong>. - (EL VINCULO). - </strong>Por la Naturaleza de la relaci&oacute;n contractual espec&iacute;fica queda establecido que <strong>{{ $contract->person->gender == 'masculino' ? 'EL CONSULTOR' : 'LA CONSULTORA' }}</strong> no recibir&aacute; beneficio adicional alguno por parte de <strong>LA ENTIDAD.</strong></p>
            ...
            @php
                $signature_date = $contract->date_presentation ?? $contract->start;
            @endphp
