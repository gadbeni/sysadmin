...
@section('content')
    <div class="content" style="text-align: justify">
        ...
        <p>Conste por el presente Contrato Administrativo de prestaci&oacute;n de servicios de consultor&iacute;a de l&iacute;nea<em><strong>, </strong></em>que celebran por una parte la Gobernaci&oacute;n del Departamento Aut&oacute;nomo del Beni, a trav&eacute;s de su {!! $signature ? $signature->direccion_administrativa->nombre : 'Secretar&iacute;a Departamental de Administraci&oacute;n y Finanzas' !!}, con <strong>NIT N&ordm; 177396029</strong>, con domicilio en el edificio de Gobernaci&oacute;n en Acera Sud de la Plaza Mariscal Jos&eacute; Ballivi&aacute;n, en la ciudad de la Sant&iacute;sima Trinidad, Provincia Cercado del Departamento del Beni, representado legalmente por <strong>{{ $signature ? $signature->name : setting('firma-autorizada.name') }}</strong>, con Cédula de Identidad <strong>N&deg; {{ $signature ? $signature->ci : setting('firma-autorizada.ci') }}</strong>, conforme a <strong>Resoluci&oacute;n Administrativa de Gobernaci&oacute;n {{ $signature ? $signature->designation : setting('firma-autorizada.designation') }}</strong>, en calidad de {{ $signature ? $signature->job : setting('firma-autorizada.job') }} @if($contract->signature) y <strong>{{ $contract->signature->person->first_name }} {{ $contract->signature->person->last_name }}</strong>, con Cédula de Identidad <strong>N&deg; {{ $contract->signature->person->ci }}</strong> en su calidad de {{ $contract->signature->job ? $contract->signature->job->name : $contract->signature->cargo->Descripcion }}, conforme a <strong>Resoluci&oacute;n Administrativa de Gobernaci&oacute;n {{ $contract->signature_code }}</strong> @endif , que en adelante se denominar&aacute;n <strong>LA ENTIDAD</strong>; y de la otra parte, {{ $contract->person->gender == 'masculino' ? 'El Señor' : 'La Señora' }} </strong><strong>{{ $contract->person->first_name }} {{ $contract->person->last_name }}</strong>, con c&eacute;dula de Identidad <strong>N&deg; {{ $contract->person->ci }}</strong>, que en adelante se denominar&aacute; <strong>{{ $contract->person->gender == 'masculino' ? 'EL CONSULTOR' : 'LA CONSULTORA' }}</strong>, quienes celebran y suscriben el presente Contrato Administrativo, de acuerdo a los t&eacute;rminos y condiciones siguientes:</p>
        ...
        <p>Los gastos por pasajes, vi&aacute;ticos que se requieran para el desarrollo de las actividades {{ $contract->person->gender == 'masculino' ? 'del consultor' : 'de la consultora' }} en las provincias y/o capitales de Bolivia, ser&aacute;n proporcionados por el Gobierno Departamental Aut&oacute;nomo del Beni.</p>
