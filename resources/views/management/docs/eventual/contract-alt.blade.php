...
@section('content')
    ...
    @else
        <div class="content">
            ...
            <p><strong>CL&Aacute;USULA CUARTA. - (Objeto): </strong>El objeto del presente documento es contratar los servicios de un <strong>&ldquo;<em>{{ Str::upper($contract->name_job_alt ?? $contract->cargo->Descripcion) }}</em>&rdquo;, </strong>en el <strong><em> {{ $contract->program->class }} {{ $contract->program->name }}</em> </strong>y establecer los t&eacute;rminos y condiciones dentro de los cuales se desenvolver&aacute; el/la <strong>CONTRATADO (A) </strong>en la ejecuci&oacute;n del presente contrato.</p>
