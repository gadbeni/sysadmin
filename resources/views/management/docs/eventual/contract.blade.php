...
@section('content')
    <div class="content" style="text-align: justify">
        ...
        <p><em>La ENTIDAD, contrata los servicios del CONTRATADO para desempe&ntilde;ar la funci&oacute;n de </em><em>{{ Str::upper($contract->cargo->Descripcion) }}</em><em>, en dependencias de la/el </em><em>{{ Str::upper($contract->unidad_administrativa->Nombre) }}</em><em> dependiente de </em><em>la/el {{ Str::upper($contract->direccion_administrativa->NOMBRE) }}</em><em> con un </em><em>nivel Salarial {{ $contract->cargo->nivel->where('IdPlanilla', $contract->cargo->idPlanilla)->first()->NumNivel }}</em><em> con cargo a la partida presupuestaria {{ $contract->program->number }} (Personal Eventual), en los t&eacute;rminos y condiciones que se establecen en este contrato, debiendo coadyuvar y coordinar sus actividades con su inmediato superior y las dem&aacute;s &aacute;reas de La ENTIDAD, donde sean requeridas. El presente contrato bajo ninguna manera podr&aacute; ser motivo de transferencia, subrogaci&oacute;n, delegaci&oacute;n, total o parcialmente.</em></p>
        ...
        <p><em><strong>CLÁUSULA D&Eacute;CIMA: (DERECHOS Y OBLIGACIONES DEL CONTRATADO)</strong></em></p>
        <ul>
